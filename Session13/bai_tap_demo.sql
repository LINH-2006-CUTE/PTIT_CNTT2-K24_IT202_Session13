drop DATABASE IF EXISTS social_network_pro_ss13;
CREATE DATABASE IF NOT EXISTS social_network_pro_ss13
  CHARACTER SET utf8mb4
  COLLATE utf8mb4_unicode_ci;

USE social_network_pro_ss13;
...
-- 1. Tạo 1 trigger kiểm tra các thông tin về user trước khi thêm vào bảng:
-- - user_name không được để trống : '' // trim()
-- - password có ít nhất 6 kí tự
-- - ngày sinh phải là ngày trong quá khứ
-- nếu không hợp lệ thì thông báo lỗi tương ứng và dùng thao tác chèn bản ghi
delimiter //
CREATE 
    TRIGGER  trigger_check_option
 BEFORE INSERT ON users FOR EACH ROW 
    BEGIN
		if trim(new.username)= '' then
        signal sqlstate '45000' set message_text= 'Ten khong duoc de trong!!!';
        elseif length(new.password) < 6 then
        signal sqlstate '45000' set message_text = 'Mat khau phai tu 6 ki tu tro len';
        elseif new.birthdate > current_date() then
		signal sqlstate '45000' set message_text = 'Ngay sinh khong hop le';
		end if;
	END// 
delimiter ;

-- 2. Tạo trigger sau khi thêm mới bài đăng thành công thì tự động tạo thêm 1 bản ghi vào bảng thông báo với các nội dung sau:
-- - uer_id: mã tài khoản đăng bài 
-- - type: post
-- - content: user_id + 'vừa đăng 1 bà viết mới'
-- - is_read:: 0
-- - created_at: mặc định thời gian hiện tại
delimiter //
create trigger trigger_add_post
after insert on postsposts for each row
begin
    INSERT INTO notifications (user_id, type, content, is_read, created_at)
    VALUES (
        NEW.user_id, 
        'post', 
        CONCAT('User ', NEW.user_id, ' vừa đăng 1 bài viết mới'), 
        0, 
        NOW()
    );
END //
